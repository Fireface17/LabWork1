...
namespace LabWork1
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // ---------------------------------main task----------------------------------------
            UniformGrid TaskGrid = new(2, 3, 6);
            V2DataUGrid Task = new("Task", dateTime, TaskGrid, F);
            Console.WriteLine(Task.ToLongString("f2"));
            V2DataList NewList = (V2DataList)Task;
            Console.WriteLine(NewList.ToLongString("f2"));
            V2DataCollection TaskColl = new();
            TaskColl.AddDefaults();
            Console.WriteLine(TaskColl.ToLongString("f2"));
            Console.WriteLine("MinDistance\n");
            foreach(V2Data v2data in TaskColl)
            {
                Console.WriteLine(v2data.MinDistance);
            }
        }
